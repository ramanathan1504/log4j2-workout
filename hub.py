...
hub.py — one local site over all three repos.
    ...
    oss run hub                 serve on http://localhost:1504, and open it
...
/* Teal and brass, the same values as site/index.html and `oss serve`. This page
   ...
   --ok, del on --bad. */
/* Light was flat: a card sat 1.09 against the page, so every card, table and code
   block read as one sheet. The page steps down and cards go white -- 1.21 raised
   against bg, 1.37 against sunken -- and --acc, which failed AA at 4.48, is the
   darker gold. Same values as the landing page, which is the point. */
:root{--bg:#E4EBED;--fg:#08161D;--mut:#4C626C;--line:#BFD0D4;--card:#FFFFFF;
--acc:#7A5D0C;--ok:#175A52;--bad:#7E3320;--code:#D3DEE1;
--warn:#8F4C16;--add:#D2E9E1;--del:#F2D5CC;--gut:#DCE5E7;}
...
# The same mapping for light, which it never had.
#
# `css = "" if theme == "light"` left the report wearing its own palette: a third
# design on a machine that already has two, and the one a visitor sees by default
# on a laptop set to light. Dark got a mapping because dark was the obvious gap;
# light was left as "fine, it was designed light" -- which is only true if the
# only thing that matters is legibility, and not that it belongs to the same tool.
TRIAGE_LIGHT = """
:root{color-scheme:light}
body{background:#E4EBED;color:#08161D}
a{color:#7A5D0C}
h3{color:#08161D}
h2{border-bottom-color:#BFD0D4}
.hint,.stat-l,.ai-note,footer{color:#4C626C}
#sidebar{background:#FFFFFF;color:#334A55}
.sb-repo a{color:#7A5D0C}
.sb-meta,.sb-sec,.sb-n{color:#4C626C}
.sb-hr{border-top-color:#BFD0D4}
.nav-list a{color:#334A55}
.nav-list a:hover{background:#D3DEE1;color:#08161D}
.sb-n{background:#D3DEE1}
table,.ai-table{background:#FFFFFF;border-color:#BFD0D4}
th{background:#D3DEE1;color:#334A55;border-bottom-color:#BFD0D4}
td{border-top-color:#BFD0D4}
tr:hover td,.ai-table tr:hover td{background:#D3DEE1}
.stat,.cluster,.ai{background:#FFFFFF;border-color:#BFD0D4}
.cluster{border-left-color:#7A5D0C}
.ai{background:#FFFFFF;border-left-color:#6D4AA8}
.ai-table th{background:#EDE7F5;color:#4A3480;border-bottom-color:#CFC2E6}
.ai-table code{background:#D3DEE1;border-color:#BFD0D4;color:#08161D}
.rr{color:#334A55;border-top-color:#BFD0D4}
.rl-bar{background:#BFD0D4}
.warn{background:#F6EAD5;border-color:#D8B77A;color:#8F4C16}
footer{border-top-color:#BFD0D4}
.b{border-width:1px;border-style:solid}
.g{background:#D2E9E1;color:#175A52;border-color:#9CCFC2}
.r{background:#F2D5CC;color:#7E3320;border-color:#D9A899}
.y{background:#F6EAD5;color:#8F4C16;border-color:#D8B77A}
.u{background:#D9E4F5;color:#274C86;border-color:#A9C1E4}
.s{background:#D3DEE1;color:#4C626C;border-color:#BFD0D4}
.p{background:#EDE7F5;color:#4A3480;border-color:#CFC2E6}
"""
...
def triage_page(theme="auto"):
    ...
    if not t:
        return None
    body_probe = t.read_text(encoding="utf-8", errors="replace")
    # A report generated by the current `oss backlog` already declares the shared
    # tokens and answers the system preference itself. Painting over one of those
    # would fight it: the remap sets explicit hexes at equal specificity, so the
    # report's own dark rule and this one would both be live and the later of the
    # two would win by position rather than by intent. Older reports on disk still
    # need it, which is why this is a check rather than a deletion.
    if "--bg:#E4EBED" in body_probe:
        css = ""
    elif theme == "light":
        css = TRIAGE_LIGHT
    elif theme == "dark":
        ...
        css = TRIAGE_LIGHT + "@media (prefers-color-scheme:dark){%s}" % TRIAGE_DARK
    body = body_probe
    if css:
...
def install_agent(remove=False):
    ...
    print(f"  serving http://localhost:1504/ at login, restarts if it dies")
...
def main():
    ...
    # 1504, not 8787.
    #
    # Two local pages on one machine is one too many: two web servers, two ports
    # to remember, two things started at login, and -- the day this changed -- two
    # JVMs that between them held four cores busy overnight. The palette that used
    # to own 1504 wraps `oss ext add` and `oss ext list`, which are one command
    # each in a terminal; this page is the one with work in it, so this page keeps
    # the number.
