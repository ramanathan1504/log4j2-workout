...
| **log4j-cassandra's DataStax driver 3.11 cannot reach a modern Cassandra, and does not fail — it hangs.** Cassandra 5 speaks native protocol v5, which this driver generation does not; against 4.1 the control connection also never completes here. The driver retries indefinitely rather than erroring, so the appender stalls at construction and takes the whole run with it, with no status-logger output at all. The bench bounds the work with an explicit timeout so one unusable appender cannot hide the others | `apps/db`, `configs/xml/appender-nosql.xml` |
