...
| **log4j-cassandra's DataStax driver 3.11 does not reach Cassandra 5.** The shipped driver generation negotiates native protocol v4 at most, and against a 5.0 container the connection attempt did not complete or error — the run stalled with no status-logger output, so the bench now bounds the work with an explicit timeout. Against **4.1 it does connect** (container healthy, appender reaching `writeInternal`), and fails later for the separate reason above. **Not fully diagnosed:** what made `startupInternal()` throw against 4.1 was masked by the missing guard and never captured; re-running with full StatusLogger output would settle it | `apps/db`, `configs/xml/appender-nosql.xml` |
