...
./bench followup --since <number> [--write]
----
...
| `--since`    | — | What the author pushed on that pull request after the review: commits, files, hunks, and anything said since
| `--write`    | off | With `--since`, append that as a dated section to the review file under `docs/pr-reviews/`
| `--repo`     | `apache/logging-log4j2` | Any repository
|===
...
`pushed` says the branch grew. `--since` says what grew, and `--write` puts that
in the review file so the answer is in the same place as the verdict it affects.

[source,bash]
----
./bench followup --since 4185            # read it
./bench followup --since 4185 --write    # append it to docs/pr-reviews/4185-*.md
----

The appended section carries the new commits, the files the author touched, the
files the review itself named crossed with the ones these commits changed, what
was said on the pull request since, and the diff hunks last. It is fenced by an
HTML comment holding the head SHA, so re-running `--write` at the same head does
nothing rather than appending twice.

WARNING: The `touched` column is file overlap, nothing more. Touched is not addressed. It says where to look; the verdict is still yours to change.

The obvious implementation of this is wrong, and it is worth knowing why. Asking
the compare API for `head_at_review...head_now` attributes to the author every
commit they merged in from the base branch. Measured on #4185: comparing two of
its own commits reported *20 commits and 89 files*, nearly all of them dependabot
bumps that arrived from `2.x`. So commits come from the pull request's own commit
list, which excludes base commits, and files come from each commit separately
with merges skipped — GitHub diffs a merge against its first parent, so a merge
of the base branch reports the whole base branch as its file list. Merges are
still listed, marked, and excluded from the counts.

If the reviewed head is no longer on the branch the author rebased or
force-pushed; that history is gone, so `--since` falls back to listing by date
and says on its face that it did.

NOTE: `--sync` is deliberately manual, and only correct after actually re-reading the pull request at its new head. Running it automatically would erase the signal it exists to show. `--since --write` does not sync — reading is a separate act from recording that you read.

Read-only against GitHub. `--sync` writes the ledger and `--since --write`
appends to a review file; neither writes to the upstream repository. Set
`BENCH_PR_LEDGER` to point at a different ledger, `BENCH_PR_REVIEW_DIR` at a
different review directory, and `BENCH_GH_USER` to override whose comments count
as yours.
...
== Posting upstream: the passphrase gate
...
So every outward `POST` from the hub is gated by a passphrase, checked against
the same `~/.oss-cli/upstream-guard.json` that OSS-CLI writes. One passphrase,
both doors:

[source,bash]
----
oss-cli setup --upstream-guard      # set it once
----

The browser asks for it at send time, *after* the existing confirm dialog. The
two are deliberately different kinds of gate: a confirm is a reflex anyone clicks
through, and this one cannot be satisfied by clicking.

WARNING: Refusal is the default in both directions that matter. **No passphrase set means refused, not unguarded** — otherwise the protection would depend on remembering to switch it on. The check also runs before `check_anchors`, so a refused send spends no API calls and nothing has left the machine.

Dispatching through OSS-CLI is gated the same way: `oss-ext.json` lists which
verbs write outward, and `oss-cli bench hub` asks before the child process
starts.
