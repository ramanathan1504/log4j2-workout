...
| `arbiters.json` | All six arbiters that work outside Spring, plus `Select`/`DefaultArbiter`. XML repeats an element to declare two arbiters of one type; JSON cannot repeat a key, so those become arrays — duplicate the key instead and Jackson keeps the last, silently losing a branch. There is no properties equivalent of this config at all; see `configs/properties/README.md`. |
| `layout-remaining.json` | Gelf, both CSV layouts, Rfc5424 + `LoggerFields`, Syslog, Message and Elastic's `EcsLayout`. Two traps live here: `CsvParameterLayout` NPEs on any event with no parameters, and `MessageLayout.toByteArray()` returns null so it cannot back a file appender at all. Both are explained in the XML original. |
| `appender-composite.json` | Failover, Routing, Rewrite, AppenderSet and ScriptAppenderSelector. **Emits status-logger errors on purpose** — Failover cannot be shown without a primary that fails. Expect three naming `BrokenPrimary`; anything else is real. The primary is a socket to a closed port rather than a bad file path, because `FileManager` creates parent directories while the manager is built, so a bad path fails at construction and Failover never engages. |
